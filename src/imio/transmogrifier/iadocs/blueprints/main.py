...
class Initialization(object):
    ...
    def __init__(self, transmogrifier, name, options, previous):  # noqa 901
        ...
        workingpath = get_main_path(
            safe_unicode(options.get("basepath") or ""), safe_unicode(options.get("subpath") or "")
        )
        ...
        if run_options["commit"]:
            ecfh = logging.FileHandler(
                os.path.join(workingpath, "{}_dt_input_errors_commit.log".format(site_name)), mode="a"
            )
            ecfh.setFormatter(logging.Formatter("(%(levelname).1s) %(message)s"))
            ecfh.setLevel(logging.INFO)
            e_logger.addHandler(ecfh)
            ocfh = logging.FileHandler(os.path.join(workingpath, "{}_dt_commit.log".format(site_name)), mode="a")
        ...
        if inb_types:
            ...
            for typ in inb_types:
                if not [dic for dic in inptc if dic["portal_type"] == typ]:
                    inptc.append(
                        {"portal_type": typ, "uniqueness": True, "default_expression": None, "default_number": 1}
                    )
                ...
                if "collective.behavior.internalnumber.behavior.IInternalNumberBehavior" not in fti.behaviors:
                    ...
                    fti.behaviors = tuple(
                        list(fti.behaviors) + ["collective.behavior.internalnumber.behavior.IInternalNumberBehavior"]
                    )
                    ftiModified(
                        fti, ObjectModifiedEvent(fti, DexterityFTIModificationDescription("behaviors", old_bav))
                    )
        ...
        if dtb_types:
            ...
            for typ in dtb_types:
                ...
                if "imio.dms.mail.content.behaviors.IDmsMailDataTransfer" not in fti.behaviors:
                    old_bav = tuple(fti.behaviors)
                    fti.behaviors = tuple(
                        list(fti.behaviors) + ["imio.dms.mail.content.behaviors.IDmsMailDataTransfer"]
                    )
                    ftiModified(
                        fti, ObjectModifiedEvent(fti, DexterityFTIModificationDescription("behaviors", old_bav))
                    )
                ...
                if typ in fields_conf:
                    ...
                    if not [dic for dic in rec if dic["field_name"] == "IDmsMailDataTransfer.data_transfer"]:
                        ...
                        lst.append(
                            {
                                "field_name": u"IDmsMailDataTransfer.data_transfer",
                                "read_tal_condition": None,
        ...
        self.storage["creation_date"] = str_to_date(
        ...
        for typ in ["types", "levels"]:
            ...
            self.storage["data"][key] = OrderedDict(
                [
                    (safe_unicode(t["token"]), {"name": safe_unicode(t["name"])})
                    for t in getattr(self.storage["plone"]["directory"], "organization_%s" % typ)
                ]
            )
        ...
        for param, service in (("default_contact", False), ("generic_service", True)):
            params = next(
                csv.reader(
                    [transmogrifier["config"].get(param).strip()], delimiter=" ", quotechar='"', skipinitialspace=True
                )
            )
            obj = get_obj_from_path(self.portal, path=params[0])
            if obj is None:
                obj = api.content.create(
                    self.portal.unrestrictedTraverse("/".join(params[0].split("/")[0:-1])),
                    "organization",
                    id=params[0].split("/")[-1],
        ...
        # store personnel
        (
            self.storage["data"]["p_userid_to_pers"],
            self.storage["data"]["p_euid_to_pers"],
            self.storage["data"]["p_hps"],
        ) = get_personnel(self.portal)
        # store categories
        self.storage["data"]["p_category"] = get_categories(self.portal)
        if "f" in self.storage["parts"] and "f__to_correct_only" in transmogrifier["transmogrifier"]["pipeline"].split(
            "\n"
        ):
            self.storage["data"]["p_categories"] = getUtility(
                IVocabularyFactory, "collective.classification.vocabularies:fulltree"
            )(self.portal)
        ...
        if def_cat:
            code, title = next(
                csv.reader(
                    [transmogrifier["config"].get("default_category").strip()],
                    delimiter=" ",
                    quotechar='"',
            ...
            if code not in self.storage["data"]["p_category"]:
                node = create_category(
                    self.portal.tree,
                    {"identifier": safe_unicode(code), "title": safe_unicode(title), "enabled": False},
                    event=True,
                )
                o_logger.info("Created default category {}".format(code))
                self.storage["data"]["p_category"][node.identifier] = {
                    "title": node.title,
                    "uid": node.UID(),
        ...
        if "f" in self.storage["parts"] or "s" in self.storage["parts"]:
            (
                self.storage["data"]["p_folder_uid"],
        ...
        # adapt im counter
        counter = api.portal.get_registry_record(
        ...
        if counter > 1:
            brains = self.portal.portal_catalog.unrestrictedSearchResults(
                portal_type=["dmsincoming_email", "dmsincomingmail"]
            )
            if not len(brains):
        ...
        if counter > 2:
            ...
            if len(brains) <= 1:
                api.portal.set_registry_record(
                    "collective.dms.mailcontent.browser.settings.IDmsMailConfig." "outgoingmail_number", 2
                )
        ...
        for item in self.previous:
            if is_in_part(self, self.parts) and self.related_storage is not None and self.condition(item):
                ...
                if self.store_subkey:
                    ...
                    if self.add_store_keys:
                        store_keys[u"{}{}".format(self.prefix, self.store_subkey)] = (
                            vdic and item[self.store_subkey] or u""
                        )
...
class CommonInputChecks(object):
    ...
    def __init__(self, transmogrifier, name, options, previous):
        ...
        self.cleans = next(
            csv.reader([options.get("clean_value", "").strip()], delimiter=" ", quotechar='"', skipinitialspace=True)
        )
        self.cleans = [cell.decode("utf8") for cell in self.cleans]
        self.cleans = [
            (
                tup[0],
                Expression(tup[1], transmogrifier, name, options)(None),
                tup[2],
                Expression(tup[3], transmogrifier, name, options)(None),
                Expression(tup[4], transmogrifier, name, options)(None),
            )
            for tup in pool_tuples(self.cleans, 5, "clean_value option")
            if tup[0] in fieldnames
        ]
        self.repl_nl = next(
            csv.reader(
                [options.get("replace_newline", "").strip()], delimiter=" ", quotechar='"', skipinitialspace=True
            )
        )
        ...
        self.invalids = next(
            csv.reader([options.get("invalids", "").strip()], delimiter=" ", quotechar='"', skipinitialspace=True)
        )
        ...
        self.concats = next(
            csv.reader([options.get("concatenate", "").strip()], delimiter=" ", quotechar='"', skipinitialspace=True)
        )
        self.concats = [cell.decode("utf8") for cell in self.concats]
        self.concats = [
            tup
            for tup in pool_tuples(self.concats, 4, "concat option")
            if tup[0] in fieldnames and tup[1] in fieldnames
        ]
        self.splits = next(
            csv.reader([options.get("split_text", "").strip()], delimiter=" ", quotechar='"', skipinitialspace=True)
        )
        ...
        self.splits = [
            (
                ...
                int(tup[3]),
                Expression(tup[4], transmogrifier, name, options)(None),
                Expression(tup[5], transmogrifier, name, options)(None),
                tup[6],
            )
            for tup in pool_tuples(self.splits, 7, "splits option")
            if tup[0] in fieldnames
        ]
        self.booleans = [key for key in safe_unicode(options.get("booleans", "")).split() if key in fieldnames]
        self.dates = next(
            csv.reader([options.get("dates", "").strip()], delimiter=" ", quotechar='"', skipinitialspace=True)
        )
        ...
        for item in self.previous:
            if is_in_part(self, self.parts) and self.condition(item):
                ...
                for fld, fmt, as_date in self.dates:
                    item[fld] = str_to_date(
                        item,
                        fld,
                        log_error,
                        fmt=fmt,
                        as_date=bool(int(as_date)),
    ...
    def __iter__(self):
        ...
        for main_key in orig_dic:
            ...
            if main_key in self.parent_relation and self.parent_relation[main_key]["_parent_id"] not in orig_dic:
                ...
                log_error(
                    temp_item,
                    u"Parent '{}' not found: please correct csv".format(self.parent_relation[main_key]["_parent_id"]),
                )
...
class InsertPath(object):
    ...
    def __init__(self, transmogrifier, name, options, previous):
        ...
        fieldnames = self.storage["csv"].get(self.csv_key, {}).get("fd", [])
        self.id_keys = [
            key for key in safe_unicode(options.get("id_keys", "")).split() if not fieldnames or key in fieldnames
        ]
                ...
                if "_path" in item:  # _path is already set
                    ...
                    if item["_eid"] in self.eids:
                        ...
                        if self.eids[item["_eid"]].get("path") and item["_path"] != self.eids[item["_eid"]]["path"]:
                            log_error(
                                item,
                                u"2 differents path: new '{} <> existing '{}'".format(
        ...
        for item in self.previous:
            if is_in_part(self, self.parts) and self.condition(item):
                ...
                if self.workflow_id not in obj.workflow_history:
                    log_error(
                        item,
                        "Cannot find '{}' wkf id in workflow_history: {}".format(
                            self.workflow_id, obj.workflow_history
                        ),
                    )
                ...
                if not self.replace:
                    status = {
                        ...
                        "actor": self.actor,
                        "comments": "",
                        "review_state": self.state_id,
                        "time": DateTime(item[self.date_key]),
                    }
                    wfh.append(status)
                    change = True
                if change:
            ...
            if is_in_part(self, self.parts) and self.condition(item, storage=self.storage):
                ...
                if self.store_subkey:
                    ...
                    if self.csku and subkey in self.storage["data"][self.bp_key].get(key, {}):
                        log_error(
                            item, u"Subkey '{}' of key '{}' already in '{}' data dict".format(subkey, key, self.bp_key)
                        )
